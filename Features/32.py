...
def main():
    ...
    # 处理消费次数和消费总金额
    count2 = cs1.execute("select xh,RIGHT(dd,2) from v_xfjl_atj where jyrq >'2019-01-10' and jyrq <'2020-01-10'")
    ...
    for i in range(count2):
        ...
        if jg[0] in xh and jg[1] == '食堂':
            xfzcs[xh.index(jg[0])] = xfzcs[xh.index(jg[0])] + 1
    ...
    for i in range(len(xh)):
        f.write(str(xh[i])+","+str(xfzcs[i])+","+str(gz[i])+"\n")
    f.close()
